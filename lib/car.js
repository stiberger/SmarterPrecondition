...
var Car = function () {
	 ...
	 * @todo setting of temperature through arg
	 */
	this.startAircondition = function(temp) {
		teslams.vehicles( { email: username, password: password }, function ( vehicles ) {
			if (vehicles.id == undefined) {
				...
				teslams.get_climate_state( vehicles.id, function ( cs ) {
					...
					if(cs.fan_status !== null) {
						winston.debug("Fan already running. Do nothing.");
						return;
					}
					winston.debug("Setting temperature");
					teslams.set_temperature( {id: vehicles.id, dtemp: temp, ptemp: temp}, function(e) {
						winston.info("Temparature is set at "+temp+"C");
