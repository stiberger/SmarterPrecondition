...
Calendar.prototype.getNewToken = function(callback) {
	...
	rl.question('Enter the code from that page here: ', function(code) {
		...
		that.oauth2Client.getToken(code, function(err, token) {
			...
			that.selectCalendar(function(calendarId) {
...
Calendar.prototype.getEvents = function(callback) {
	...
	calendar.events.list({
		...
		if (events.length == 0) {
			winston.info('No upcoming events found.');
		} else {
			...
			for (var i = 0; i < events.length; i++) {
				...
				//The duration of the event is more than the default - one hour.
				//Add events that will check the status of the AC and trigger it again if nescessary.
				//Can in theory work as camper mode, but not tested thoroughly.
				if(end.diff(start) > 3600000) {
					winston.debug('Duration exceeds one hour. Adding events every minute until end time.');
					while(start.add(1, 'minutes') < end) {
						callback({
							start: start.toDate(),
							end: end.toDate(),
							summary: event.summary,
...
Calendar.prototype.setEvent = function(date, callback) {
	...
	this.eventJobs.push(new cron(date, function() {
		winston.info('Running at %s', new Date());
		callback();
	}, null, true));
};
